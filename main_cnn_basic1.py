...
def train(X, y, num_epochs, learning_rate, hidden_size):
    np.random.seed(0)
    ...
    for epoch in range(num_epochs):
        ...
        if epoch % 100 == 0:
            ...
            Z2 = np.dot(A1, W2) + b2
            A2 = softmax(Z2)
            train_acc = np.mean(np.argmax(A2, axis=1) == np.argmax(y, axis=1))
...
X_train = data[:, 1:] / 255.0  # Chuẩn hóa giá trị pixel về khoảng [0, 1]
y_train = data[:, 0].astype(int)  # Nhãn lớp
...
# Huấn luyện mô hình
W1, b1, W2, b2 = train(X_train, y_one_hot, num_epochs, learning_rate, hidden_size)

...
# Chuẩn bị ảnh test.png
image_path = 'test.png'
image = np.array(cv2.imread(image_path, cv2.IMREAD_GRAYSCALE), dtype=np.uint8)

# Tiền xử lý ảnh (nếu cần)
# Ví dụ: Resize ảnh về kích thước 32x32
desired_size = (32, 32)
image = cv2.resize(image, desired_size)

# Trích xuất đặc trưng từ ảnh
input_data = [0]  # Tạo một danh sách để lưu trữ đặc trưng của ảnh
input_data.extend(image.flatten())  # Thêm các giá trị pixel vào danh sách
